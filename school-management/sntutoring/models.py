...
from sncommon.models import BaseAvailability, BaseRecurringAvailability, TimeCardBase, get_default_availability
...
class Diagnostic(CWModel):
    ...
    resources = models.ManyToManyField("snresources.Resource", related_name="diagnostics", blank=True)
...
class DiagnosticResult(CWModel):
    ...
    task = models.OneToOneField(
        "sntasks.Task", related_name="diagnostic_result", blank=True, null=True, on_delete=models.SET_NULL,
    )

    diagnostic = models.ForeignKey(
        "sntutoring.Diagnostic", related_name="diagnostic_results", on_delete=models.CASCADE,
    )
...
class DiagnosticGroupTutoringSessionRegistration(CWModel):
    ...
    group_tutoring_sessions = models.ManyToManyField(
        "sntutoring.GroupTutoringSession", related_name="diagnostic_gts_registrations"
    )
    # The self-assigned diagnostics student registered for
    self_assigned_diagnostics = models.ManyToManyField(
        "sntutoring.Diagnostic", related_name="diagnostic_gts_registrations"
    )
...
class Location(AddressFields):
    """ A physical location where tutoring OR COUNSELING is offered
        TODO: This model should probably be moved into sncommon for logical consistency since change to
...
class GroupTutoringSession(CWModel):
    ...
    resources = models.ManyToManyField("snresources.Resource", related_name="group_tutoring_sessions", blank=True)
    # If this session is administration of a diagnostic, this is the diag being administered
    diagnostic = models.ForeignKey(
        "sntutoring.Diagnostic",
        related_name="group_tutoring_sessions",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    ...
    # student_tutoring_sessions > many sntutoring.StudentTutoringSession
    # tutoring_packages > many TutoringPackage
...
class Course(CWModel):
    ...
    resources = models.ManyToManyField("snresources.Resource", related_name="courses", blank=True)
    ...
    package = models.ForeignKey(
        "sntutoring.TutoringPackage", related_name="courses", null=True, blank=True, on_delete=models.SET_NULL,
    )
    ...
    location = models.ForeignKey(
        "sntutoring.location", related_name="courses", on_delete=models.PROTECT, default=get_default_location,
    )

    group_tutoring_sessions = models.ManyToManyField(
        "sntutoring.GroupTutoringSession", related_name="courses", blank=True
    )
...
class StudentTutoringSession(CWModel):
    ...
    group_tutoring_session = models.ForeignKey(
        "sntutoring.GroupTutoringSession",
        related_name="student_tutoring_sessions",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    ...
    set_resources = models.ManyToManyField("snresources.Resource", related_name="student_tutoring_sessions")
...
class TutoringService(CWModel):
    ...
    locations = models.ManyToManyField("sntutoring.Location", related_name="tutoring_services", blank=True)
...
class TutoringSessionNotes(CWModel):
    ...
    resources = models.ManyToManyField("snresources.Resource", related_name="tutoring_session_notes")
    ...
    """ Incoming Foreign Keys """
    # file_uploads > many sncommon.FileUpload
    # student_tutoring_sessions > many sntutoring.StudentTutoringSession
...
class TutoringPackage(CWModel):
    ...
    locations = models.ManyToManyField("sntutoring.Location", related_name="tutoring_packages", blank=True)
    ...
    group_tutoring_sessions = models.ManyToManyField(
        "sntutoring.GroupTutoringSession", related_name="tutoring_packages", blank=True
    )

    # Resources made available to student upon purchasing this package
    resource_groups = models.ManyToManyField("snresources.ResourceGroup", related_name="tutoring_packages", blank=True)
...
class TutoringPackagePurchase(CWModel):
    ...
    tutoring_package = models.ForeignKey(
        "sntutoring.TutoringPackage", related_name="tutoring_package_purchases", on_delete=models.CASCADE,
    )
...
class TutorTimeCard(TimeCardBase):
    """ A timecard (usually over the course of a week) with line-item entries for a tutor's
        ...
        a year apart, there is a separate time card model for counselors (in sncounseling): CounselorTimeCard
    """
...
class TutorTimeCardLineItem(CWModel):
    ...
    individual_tutoring_session = models.ForeignKey(
        "sntutoring.StudentTutoringSession",
        related_name="time_card_line_items",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    )
    time_card = models.ForeignKey("sntutoring.TutorTimeCard", related_name="line_items", on_delete=models.CASCADE)
