...
from cwcommon.models import BaseAvailability, BaseRecurringAvailability, TimeCardBase

from cwcounseling.constants import (
...
class AgendaItemTemplate(CWModel):
    ...
    pre_meeting_task_templates = models.ManyToManyField(
        "cwtasks.TaskTemplate", related_name="pre_agenda_item_templates", blank=True
    )
    post_meeting_task_templates = models.ManyToManyField(
        "cwtasks.TaskTemplate", related_name="post_agenda_item_templates", blank=True
    )
...
class AgendaItem(CWModel):
    ...
    agenda_item_template = models.ForeignKey(
        "cwcounseling.AgendaItemTemplate",
    ...
    counselor_meeting = models.ForeignKey(
        "cwcounseling.CounselorMeeting", related_name="agenda_items", on_delete=models.CASCADE
    )
    order = models.IntegerField(default=1)
...
class CounselorMeetingTemplate(CWModel):
    ...
    counselor_resources = models.ManyToManyField(
        "cwresources.Resource", related_name="counselor_meeting_template_resources", blank=True
    )

    roadmap = models.ForeignKey(
        "cwcounseling.Roadmap",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
...
class CounselorMeeting(CWModel):
    ...
    event_type = models.ForeignKey(
        "cwcounseling.CounselorEventType",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="counselor_meetings",
    ...
    student_resources = models.ManyToManyField(
        "cwresources.Resource", related_name="counselor_meeting_student_resources", blank=True
    )
    ...
    notes_message_upcoming_tasks = models.ManyToManyField("cwtasks.Task", related_name="+", blank=True)
    notes_message_completed_tasks = models.ManyToManyField("cwtasks.Task", related_name="+", blank=True)
    ...
    location = models.ForeignKey("cwtutoring.Location", null=True, blank=True, on_delete=models.SET_NULL)
...
class CounselorNote(CWModel):
    ...
    counselor_meeting = models.ForeignKey(
        "cwcounseling.CounselorMeeting", related_name="counselor_notes", null=True, blank=True, on_delete=models.CASCADE
    )
...
class CounselingHoursGrant(CWModel):
    ...
    counseling_package = models.ForeignKey(
        "cwcounseling.CounselingPackage",
        related_name="counseling_hours_grants",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
...
class CounselorTimeEntry(CWModel):
    ...
    counselor_time_card = models.ForeignKey(
        "cwcounseling.CounselorTimeCard",
